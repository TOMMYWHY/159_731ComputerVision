...
//    imshow("org_img",img);
    ...
    vector<double> feature_values = get_feature(obj_img);
    vector<vector<double> >feature_list_all;
    feature_list_all.push_back(feature_values);
...
/* 写 文件*/
    ...
    for (int i = 0; i < feature_list_all.size(); i++) {
        for (int j = 0; j < feature_list_all[i].size(); j++) {
            cout<<feature_list_all[i][j] <<"," ;
            w_file << feature_list_all[i][j]<<",";
        }
        w_file<<endl;
    }
