...
//    iLowH:5iLowS:17iLowV:51iHighH:179iHighS:248iHighV:113
    inRange(hsv,Scalar(5,17,51),Scalar(179,248,113),binary_img);
...
int main (int argc, char *argv[]){
    string model_name = "./res/mlp_model.xml";
...
//     flag = "s";//static
 ...
 /* step1: Load trainning images and get CE features */
        cout << "model:" <<"train" <<endl;
    LoadData all_img("./images/train_img/","./res/train.data",0);
    }
    else if (flag=="b_test"){
    LoadData all_img("./images/test_img/","./res/test.data",1);

    }
    else if(flag=="T"){
 /* step2: build model */
        cout << "model:" <<"train" <<endl;
//        LoadData all_img("./images/train_img/","./res/train.data");
        build_mlp_classifier( "./res/train.data", "./res/mlp_model.xml", "" );
    }
    else if(flag=="t"){
        ////-load =>test
        cout << "model:" <<"test" <<endl;
        build_mlp_classifier( "./res/test.data", "", "./res/mlp_model.xml" );
        waitKey(0);
    }
    else if(flag=="s"){
        //// /* step3:single file testing */
        cout << "model:" <<"static" <<endl;
    Mat test_img = imread("./images/test_img/0_B.jpg",1);//0
        ...
        Ptr<ANN_MLP> model;
        model = load_classifier<ANN_MLP>("./res/mlp_model.xml");
        Mat sample1 = (Mat_<float>(1,19) << CE[1],CE[2],CE[3],CE[4],CE[5],CE[6],CE[7],CE[8],CE[9],CE[10],
...
////    /* step4: predict webcam */
        ...
        capture.open(0);
        capture.set( CAP_PROP_FRAME_WIDTH, 680) ;
