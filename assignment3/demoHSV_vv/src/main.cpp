...
void single_test(Mat test_img,string model_file){
    ...
    vector<float> CE = p_sample->feature;
    Mat test_sample = (Mat_<float>(1,p_sample->feature_amount-1) << CE[1],CE[2],CE[3],CE[4],CE[5],CE[6],CE[7],CE[8],CE[9],CE[10],
...
Mat color_distinguish(Mat srcImage){
    namedWindow("color");
    ...
    Canny(imgThresholded,imgCanny,20,60);
     ...
     if(command=="train"){
        /* step2: build model */
         ...
         while(1){
             ...
             Mat binary_img = color_distinguish(imageROI);
             cout <<"iLowH:"<<iLowH
                  <<"iLowS:"<<iLowS
                  <<"iLowV:"<<iLowV
                  <<"iHighH:"<<iHighH
