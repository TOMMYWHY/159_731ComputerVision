...
//    Mat input_img = imread("./images/congratulations_rotated_scaled.jpg",1);
    ...
    vector<Point2f> inputPoints = {input_circles[0],input_circles[1],input_circles[2]}; // 无序点
    cout << "inputPoints 无序 ------ : "<<inputPoints << endl;

    vector<Point2f> new_input_pos_vec;
    new_input_pos_vec.push_back( srcTri[input_right_angle]); // 先放入直角点
    /*确保 直角点 在 1 号位*/
    if(input_right_angle==0){
        new_input_pos_vec.insert(new_input_pos_vec.begin(),inputPoints[1]);
        new_input_pos_vec.push_back(inputPoints[2]);
    }else if(input_right_angle==1){
        new_input_pos_vec.insert(new_input_pos_vec.begin(),inputPoints[0]);
        new_input_pos_vec.push_back(inputPoints[2]);
    }else{
        new_input_pos_vec.insert(new_input_pos_vec.begin(),inputPoints[0]);
        new_input_pos_vec.push_back(inputPoints[1]);
    }
/*empy 斜边斜率*/
    float emp_k = (dstTri[0].y-dstTri[2].y) /(dstTri[0].x-dstTri[2].x);
    float input_k = (new_input_pos_vec[0].y-new_input_pos_vec[2].y) /(new_input_pos_vec[0].x-new_input_pos_vec[2].x);
    cout << "input_k : "<<input_k << endl;
//    cout << "可以看出 opencv 坐标与正规坐标区别 emp_k : "<<emp_k << endl;
    cout << "new_input_pos_vec : "<<new_input_pos_vec << endl;

    /*此处 k < 0 是以 opencv 坐标~~~~！！！！*/ //todo 尝试将 y 值 加负号
/*    if(input_k < 0 &&   new_input_pos_vec[1].y - input_k *  new_input_pos_vec[1].x > 0 ){
        if(new_input_pos_vec[0].x < new_input_pos_vec[1].x){
            cout << "new_input_pos_vec[0] 好像猜对了~！:"<< new_input_pos_vec[0]<<endl;
        }else{
            swap(new_input_pos_vec[0],new_input_pos_vec[2]);
        }
    }else if(input_k < 0 &&   new_input_pos_vec[1].y - input_k *  new_input_pos_vec[1].x < 0 ){
        if(new_input_pos_vec[0].x > new_input_pos_vec[1].x){
            cout << "new_input_pos_vec[0] 好像猜对了~！:"<< new_input_pos_vec[0]<<endl;
        }else{
            swap(new_input_pos_vec[0],new_input_pos_vec[2] );
        ...
        if(new_input_pos_vec[0].y > new_input_pos_vec[1].y){
            cout << "new_input_pos_vec[0] 好像猜对了~！:"<< new_input_pos_vec[0]<<endl;
        }else{
            swap(new_input_pos_vec[0],new_input_pos_vec[2] );
        }
...
//    cout << "after swap new_input_pos : "<<new_input_pos << endl; // 此处 input 点 顺序 与 empty 一致~！！！

    swap(new_input_pos_vec[0],new_input_pos_vec[2]);
    cout << "after swap new_input_pos_vec : "<<new_input_pos_vec << endl; // 此处 input 点 顺序 与 empty 一致~！！！

    Point2f new_input_position[3] ={new_input_pos_vec[0],new_input_pos_vec[1],new_input_pos_vec[2]}; //转成 数组 todo 尝试 向量

    Mat  input_tuning;
    Mat warp_dst = Mat::zeros( emp_img.rows, emp_img.cols, emp_img.type() );
    Mat rotate_Mat = getAffineTransform( new_input_position, dstTri  );
    warpAffine( input_img, input_tuning, rotate_Mat, warp_dst.size() );

    imshow("input_tuning",input_tuning);
